% -------------------------------------------------------------------------------------------------
function bboxes = tracker(base_path,sequence_name,varargin)
%TRACKER
...
% -------------------------------------------------------------------------------------------------
    ...
    p = env_paths_tracking(base_path,p);
...
%     end
    ...
    for i = startFrame:nImgs
        ...
        if p.visualization
            if i==1,
                figure('Number','off', 'Name',['Tracker - ' 'SiamFC']);
                im_handle = imshow(uint8(im), 'Border','tight', 'InitialMag', 100 + 100 * (length(uint8(im)) < 500));
                rect_handle = rectangle('Position',rectPosition, 'EdgeColor','g');
                text_handle = text(10, 10, int2str(i));
                set(text_handle, 'color', [0 1 1]);
            else
                im = gather(im)/255;
                set(im_handle, 'CData', im);
                set(rect_handle, 'Position', rectPosition);
